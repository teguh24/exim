  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form class="form" method="POST" action="{{asset('supplier')}}">
            @csrf
...
<div class="modal fade" id="edit{{$row->id}}">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <!-- Modal body -->              
        <div class="modal-body">
        <form class="form" method="POST" action="{{ asset('supplier/'.$row->id) }}">
            @method('PUT')
            @csrf
            <label>supplier</label>
            <input type="text" class="form-control" value="{{$row->supplier}}" name="supplier" placeholder="supplier">        
      </div>
      <!-- Modal footer -->
      <div class="modal-footer">
        <input type="submit" class="btn btn-success">
                    ...
                    <table id="import" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{$row->supplier}}</td>
                                <td>
                                    <form id="myform" method="POST" action="{{ asset('supplier/'.$row->id) }}">
                                        @method('DELETE')
                                        @csrf
