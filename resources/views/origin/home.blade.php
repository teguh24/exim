  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <h4 class="modal-title">Create Origin</h4>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>

      <!-- Modal body -->
      <div class="modal-body">
        <form class="form" method="POST" action="{{asset('origin')}}">
            @csrf
            <label>Origin</label>
...
<div class="modal fade" id="edit{{$row->id}}">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <!-- Modal body -->              
        <div class="modal-body">
        <form class="form" method="POST" action="{{ asset('origin/'.$row->id) }}">
            @method('PUT')
            @csrf
            <label>Origin</label>
            <input type="text" class="form-control" value="{{$row->origin}}" name="origin" placeholder="Origin">        
      </div>
      <!-- Modal footer -->
      <div class="modal-footer">
        <input type="submit" class="btn btn-success">
                    ...
                    <table id="import" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{$row->origin}}</td>
                                <td>
                                    <form id="myform" method="POST" action="{{ asset('origin/'.$row->id) }}">
                                        @method('DELETE')
                                        @csrf
